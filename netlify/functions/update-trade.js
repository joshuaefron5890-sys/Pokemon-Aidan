...
// Body: { tradeId, action: "withdraw"|"accept"|"reject", binderSlug }
// - "withdraw" used by initiator; finds trade in sent list
// - "accept"|"reject" used by binder owner; finds trade in received list
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!newStatus || !tradeId) {
      return { statusCode: 400, body: JSON.stringify({ error: "Invalid action or missing tradeId" }) };
    }
    ...
    if (action === "withdraw") {
      ...
      // accept / reject — caller is the binder owner
